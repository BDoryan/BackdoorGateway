	...
	public void join(Group group) {
		...
		if(index == team_count) {
			...
			players.forEach((client) -> {
				try {
					client.sendPacket(new PacketGroupMessage(PacketGroupMessage.SUCCESS, "lang:find_game"));
				} catch (IOException e) {
					e.printStackTrace();
				}
			});
			...
			players.forEach((client) -> {
				try {
					client.sendPacket(new PacketGroupMessage(PacketGroupMessage.SUCCESS, "lang:connection_to_the_game_server"));
				} catch (IOException e) {
					e.printStackTrace();
				}
			});
			...
			players.forEach((client) -> {
				try {
					client.sendPacket(new PacketGroupMessage(PacketGroupMessage.SUCCESS, ""));
				} catch (IOException e) {
					e.printStackTrace();
				}
			});
		...
		group.setQueue(this);
		group.message(PacketGroupMessage.SUCCESS, "lang:search_for_a_game", -1);
